...
- **职业化技能体系**：技能卡按职业绑定，玩家同职业角色可游戏中切换
- **移动端轻量同步**：玩家端使用精简数据接口与串行保存队列，避免覆盖和回滚
...
interface ICard {
  ...
  profession?: string;    // 职业（玩家卡必填，同职业共享技能卡）
...
interface IPlayer {
  ...
  availableRoles: any[];  // 可切换的同职业角色卡列表
  profession: string;     // 所属职业
  name: string;           // 角色名
  imgUrl: string;         // 头像
  color: string;          // 玩家颜色标识
  hp: number;             // 当前血量
...
interface IGameSession {
  ...
  // 多玩家支持
  players: IPlayer[];
  roleId?: ObjectId;      // 兼容字段，可为空
  roleName?: string;      // 兼容字段，可为空
  roleImgUrl?: string;    // 兼容字段，可为空
...
**Query 参数**：
- `lite=true` 只返回玩家数据及公共/资源牌堆、公共弃牌、战役名、是否已开始（移动端使用）
...
- 技能卡支持按职业筛选（含“无职业”），分页/筛选/类型记忆
- 技能卡列表可直接查看并批量修改张数（就地编辑后一次保存）
...
- 地图弃牌堆
- 地图标记功能
- 职业化玩家：按职业分配技能牌堆，玩家可在同职业内切换角色卡
- 头像/弃牌等图片支持放大查看

### 7. 玩家移动端页面 (`/game/player`)
功能：
- 移动端适配的玩家界面
- 使用轻量接口实时同步，仅加载必要数据，避免覆盖地图等状态
- 查看手牌和装备
- 抽牌（红/蓝/绿/商店/技能）
- 管理自己的卡牌
- 保存操作串行队列，防止快速出牌状态回滚
...
### Q: 技能卡如何分配给玩家？
A: 技能卡不在战役中配置，而是根据职业自动分配；同职业的玩家角色共享同一套技能卡（卡牌 `role` 存职业名）。
...
- **v0.4.0**: 职业化与移动端优化
  - 技能卡按职业绑定，玩家卡新增职业字段；同职业可游戏中切换角色
  - 游戏设置页按职业选择并加载对应角色
  - 玩家/存档模型增加职业与可切换角色列表，角色字段兼容可为空
  - 玩家端使用 `/api/game-sessions/[id]?lite=true` 精简加载与同步，避免地图覆盖
  - 玩家端保存请求串行队列，解决快速出牌回滚
  - 头像/弃牌/手牌支持放大查看
  - 管理端技能卡职业筛选（含无职业）、分页/类型记忆，列表内快速编辑张数并批量保存
