...
import { ChevronLeft, Check, X, Plus, ChevronDown, ChevronRight, Search } from 'lucide-react';
...
export default function NewCampaignPage() {
  ...
  // 版本展开状态 (key: "type-edition", value: boolean)
        ...
        {paginatedCards.map((card) => {
            ...
            return (
              ...
              >
                <div className="aspect-square bg-black rounded overflow-hidden mb-2 relative">
                  ...
                  />
                  {cardConfsList.length > 0 && (
                    <div className="absolute top-1 right-1 bg-purple-600 text-white text-[10px] px-1.5 py-0.5 rounded font-bold">
          ...
          return (
            <div 
              ...
              ${isSelected
                  ? 'border-purple-500 bg-purple-900/20 shadow-[0_0_15px_rgba(168,85,247,0.3)] scale-105 z-10' 
  ...
  if (loading) return <div className="min-h-screen bg-slate-950 flex items-center justify-center"><div className="text-amber-500 animate-pulse">正在加载卡牌数据...</div></div>;
        ...
        <div className="mb-8 bg-slate-900/80 backdrop-blur-md p-6 rounded-xl border border-slate-700 shadow-xl sticky top-4 z-20">
          <div className="flex gap-6 items-end mb-4">
            <div className="flex-1">
              ...
              <input 
                ...
                className="w-full bg-slate-800 border border-slate-600 rounded p-3 text-white focus:ring-2 focus:ring-purple-500 outline-none placeholder-slate-600"
            ...
            <div className="w-64">
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full bg-slate-800 border border-slate-600 rounded p-3 pl-10 text-white focus:ring-2 focus:ring-purple-500 outline-none placeholder-slate-600"
                  placeholder="输入卡牌名称..."
                />
                {searchQuery && (
                  <button 
            ...
            <button 
              ...
              disabled={submitting}
              className="bg-purple-600 text-white px-8 py-3 rounded-lg hover:bg-purple-500 disabled:opacity-50 font-bold shadow-lg hover:shadow-purple-900/50 transition-all"
            >
              {submitting ? '保存中...' : '创建战役'}
            </button>
          </div>
          ...
          {getResourceConfigs().length > 0 && (
             <div className="mt-4 pt-4 border-t border-slate-700">
                <div className="text-sm font-bold text-slate-400 mb-2">已选资源卡配置:</div>
                <div className="flex flex-wrap gap-2">
                    {getResourceConfigs().map(config => {
          ...
          {getEnemyConfigs().length > 0 && (
             <div className="mt-4 pt-4 border-t border-slate-700">
                <div className="text-sm font-bold text-slate-400 mb-2">已选敌人卡配置:</div>
                <div className="flex flex-wrap gap-2">
                    {getEnemyConfigs().map(config => {
                        const card = cards.find(c => c._id === config.cardId);
                        if (!card) return null;
                        return (
